...
check=disable: disable health check`)
	flag.IntVar(&conf.Strategy.CheckInterval, "checkinterval", 30, "fowarder check interval in seconds")
	flag.IntVar(&conf.Strategy.CheckTimeout, "checktimeout", 10000, "fowarder check timeout in milliseconds")
	flag.IntVar(&conf.Strategy.CheckTolerance, "checktolerance", 0, "fowarder check tolerance in milliseconds, switch only when new_latency < old_latency - tolerance, only used in lha mode")
	...
	flag.IntVar(&conf.Strategy.DialTimeout, "dialtimeout", 3000, "dial timeout in milliseconds")
	flag.IntVar(&conf.Strategy.RelayTimeout, "relaytimeout", 0, "relay timeout in milliseconds")
	...
	flag.IntVar(&conf.DNSConfig.Timeout, "dnstimeout", 3000, "timeout value used in multiple dnsservers switch in milliseconds")
