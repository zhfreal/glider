...
func NewConfFromFile(ruleFile string) (*Config, error) {
	...
	f.IntVar(&p.Strategy.CheckInterval, "checkinterval", 30, "fowarder check interval in seconds")
	f.IntVar(&p.Strategy.CheckTimeout, "checktimeout", 10000, "fowarder check timeout in milliseconds")
	f.IntVar(&p.Strategy.CheckLatencySamples, "checklatencysamples", 10, "use the average latency of the latest N checks")
	f.IntVar(&p.Strategy.CheckTolerance, "checktolerance", 0, "fowarder check tolerance in milliseconds, switch to new forwarder only when new_latency < old_latency - tolerance, only used in lha mode")
	...
	f.IntVar(&p.Strategy.DialTimeout, "dialtimeout", 3000, "dial timeout in milliseconds")
	f.IntVar(&p.Strategy.RelayTimeout, "relaytimeout", 0, "relay timeout in milliseconds")
